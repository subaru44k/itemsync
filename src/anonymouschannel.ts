...
const channelArea = new Vue({
    ...
    methods: {
        ...
        handleAddItem: function() {
            firebaseControl.addItemForAnonymousChannel(channelId, "new item", "anonymous");
        },
        handleUpdateItem: function(item: Item) {
            firebaseControl.updateItemForAnonymousChannel(channelId, item, "anonymous");
        },
        handleDeleteItem: function(item: Item) {
            firebaseControl.deleteItemForAnonymousChannel(channelId, item.getId());
        },
    },
    components: {
...
firebaseControl.getAnonymousChannel(channelId).then((channelDoc) => {
    channelName.pop();
    channelName.push(channelDoc.data()['name']);
});
firebaseControl.listenAnonymousChannelChange(channelId, new DefaultFirebaseCallback(itemList));
