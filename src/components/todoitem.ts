const templateTags = ''
+ '<div class="col-xl-3 col-md-4 col-sm-6 pr-2 pl-2 pt-1 pb-1">'
+ '  <div class="alert item-color" role="alert" v-on:click="onItemClicked(item)">'
+ '    <div class="right-align">'
+ '      <img src="/images/checked.svg" v-on:click.stop="onDeleteClicked(item)">'
+ '    </div>'
+ '    <h3 class="m-0 inline-block" v-if="!isInputVisible">{{ item.getContent() }}</h3>'
...
+ '    <div class="right-align">'
+ '      <p class="font-weght-light font-italic m-0"><small>{{ item.getCreatedBy() }}</small></p>'
+ '    </div>'
+ '  </div>'
+ '</div>';
